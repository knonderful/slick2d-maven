 ...
 * effect. The approach is very efficient and pretty flexible. It can be
 * extended in many ways.
 *
 * The down side is that the resolution of your lighting map is the same as that
 * of your tile map. So the small area you'll see a change in lighting across is
 * 32x32 pixels in this case (the size of a single tile). This can be worked
 * round by rendering black tiles over an existing map at a different
 * resolution. For instance, you may use 16x16 black tiles over a 32x32 pixels
 * tiled map to get a better resolution of lighting.
 *
 * This example essentially generates a random map of tiles, creates a set of
 * lights and calculates their effect on each of vertexs in the tiled map. When
 * rendering we apply those calculated values to the tile image vertex colours
 * before rendering it. This gives us the effect of lighting the tiles.
 *
 * @author kevin
 */
public class LightTest extends BasicGame {
  ...
  /**
   * The main light that we'll move around with the mouse, held seperately so we
   * can update it
   */
  ...
  /**
   * Update the vertex values for lighting based on the current light
   * configuration.
   */
