...
public class TTFFile {
   ...
   * unsupported tables are present. Currently only unicode cmaps are supported.
  ...
  /**
   * Read the "hhea" table to find the ascender and descender and size of "hmtx"
   * table, as a fixed size font might have only one width.
   *
   ...
   * problem here is that the interpretation of these values is not the same for
   * every font. There doesn't seem to be a uniform definition of an ascender
   * and a descender. In some fonts the hhea values are defined after the Apple
   * interpretation, but not in every font. The same problem is in the OS/2
   * table. FOP needs the ascender and descender to determine the baseline so we
   * need values which add up more or less to the "em box". However, due to
   ...
   * the collection. If it does, set offset in fontfile to the beginning of the
   * Table Directory for that font.
   *
   * @param in FontFileReader to read from
