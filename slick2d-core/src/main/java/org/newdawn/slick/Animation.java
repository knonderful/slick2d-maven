...
public class Animation implements Renderable {
   ...
   */
  public Animation(Renderable[] frames, int duration) {
   ...
   */
  public Animation(Renderable[] frames, int[] durations) {
   ...
   * Create an empty animation
   *
   * @param autoUpdate True if this animation should automatically update. This
   * means that the current frame will be caculated based on the time between
   * renders
   */
   ...
   * renders
   */
  public Animation(Renderable[] frames, int duration, boolean autoUpdate) {
   ...
   * renders
   */
  public Animation(Renderable[] frames, int[] durations, boolean autoUpdate) {
    this.autoUpdate = autoUpdate;
   ...
   * the sprites are organised on horizontal scan lines and that every sprite in
   * the sheet should be used.
   *
   ...
   *
   * @param frames The sprite sheet containing the frames
   * @param x1 The x coordinate of the first sprite from the sheet to appear in
   * the animation
   * @param y1 The y coordinate of the first sprite from the sheet to appear in
   * the animation
   * @param x2 The x coordinate of the last sprite from the sheet to appear in
   * the animation
   * @param y2 The y coordinate of the last sprite from the sheet to appear in
   * the animation
   * @param horizontalScan True if the sprites are arranged in hoizontal scan
   * lines. Otherwise vertical is assumed
   * @param duration The duration each frame should be displayed for
   * @param autoUpdate True if this animation should automatically update based
   * on the render times
   */
   ...
   */
  public void addFrame(Renderable frame, int duration) {
    if (duration == 0) {
  ...
  /**
   * Render the appropriate frame when the spriteSheet backing this Animation is
   * in use.
   *
   * @param x The x position to draw the animation at
   * @param y The y position to draw the animation at
   ...
   */
  public Renderable getCurrentFrame() {
    Frame frame = (Frame) frames.get(currentFrame);
    return frame.image;
  }
  ...
  private class Frame {
     ...
     */
    public Frame(Renderable image, int duration) {
      this.image = image;
