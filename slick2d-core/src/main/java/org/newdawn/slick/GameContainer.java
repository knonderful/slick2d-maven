...
public abstract class GameContainer implements GUIContext {
  ...
  /**
   * Indicate if we should force exitting the VM at the end of the game (default
   * = true)
   *
   ...
   *
   * @return The context shared by all the containers or null if shared context
   * isn't enabled
   */
   ...
   * you're rendering to the whole screen each frame then setting this to false
   * can give some performance improvements
   *
   ...
   * @throws SlickException Indicates a failure to load the cursor image or
   * create the hardware cursor
   */
   ...
   * will not take account of render state type changes to images (rotation and
   * such). If these effects are required it is recommended that an offscreen
   * buffer be used to produce an appropriate image. An offscreen buffer will
   * always be used to produce the new cursor and as such this operation an be
   * very expensive
   *
  ...
  /**
   * Check if the mouse cursor is current grabbed. This will cause it not to be
   * seen.
   *
   ...
   * default is true)
   *
   * @param updateOnlyWhenVisible True if we should updated only when the
   * display is visible
   */
