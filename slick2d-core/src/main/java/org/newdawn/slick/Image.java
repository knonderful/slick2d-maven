...
public class Image implements Renderable {
   ...
   * @throws SlickException Indicates a failure to create the underlying
   * resource
   */
  ...
  /**
   * Get the reference to the resource this image was loaded from, if any. Note
   * that this can be null in the cases where an image was programatically
   * generated.
   *
   ...
   *
   * @param x The x coordinate of center of rotation relative to the top left
   * corner of the image
   * @param y The y coordinate of center of rotation relative to the top left
   * corner of the image
   */
  ...
  /**
   * Set the angle to rotate this image to. The angle will be normalized to be 0
   * <= angle < 360. The image will be rotated around its center.
   *
  ...
  /**
   * Get the current angle of rotation for this image. The image will be rotated
   * around its center.
   *
...
//		GL.glEnd();
  ...
  /**
   * Draw a section of this image at a particular location and scale on the
   * screen, while this is image is "in use", i.e. between calls to startUse and
   * endUse.
   *
   * @param x The x position to draw the image
   * @param y The y position to draw the image
  ...
  /**
   * <p>
   * Get a scaled copy of this image.</p>
   *
   * <p>
   * This will only scale the <strong>canvas</strong>, it will not scale the
   * underlying texture data. For a downscale, the texture will get clipped. For
   * an upscale, the texture will be repetated on both axis</p>
   *
   * <p>
   * Also note that the underlying texture might be bigger than the initial
   * image that was loaded because its size is fixed to the next power of two of
   * the size of the image. So if a 100x100 image was loaded, the underlying
   * texture is 128x128. If it's then scaled to 200x200, there will be a gap
   * between 100 to 128 (black), and then the texture will be repeated.</p>
   *
   * <img src="doc-files/Image-getScaledCopy.png" />
   *
   * <p>
   * This especially has nasty side effects when you scale a flipped
   * image...</p>
   *
  ...
  /**
   * Start using this sheet. This method can be used for optimal rendering of a
   * collection of sprites from a single sprite sheet. First, startUse(). Then
   * render each sprite by calling renderInUse(). Finally, endUse(). Between
   * start and end there can be no rendering of other sprites since the
   * rendering is locked for this sprite sheet.
   */
   ...
   *
   * @throws SlickException Indicates a failure to release resources on the
   * graphics card
   */
