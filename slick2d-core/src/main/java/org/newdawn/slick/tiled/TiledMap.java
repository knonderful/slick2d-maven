...
public class TiledMap {
   ...
   *
   * @param ref The location of the tile map to load
   * @param loadTileSets True if we want to load tilesets - including their
   * image data
   * @throws SlickException Indicates a failure to load the tilemap
   */
   ...
   * @param layerIndex The index of the layer on which the tile whose image
   * should be retrieve exists
   ...
   * @return The value assigned to the property on the map (or the default value
   * if none is supplied)
   */
  ...
  /**
   * Get a property given to a particular layer. Note that this method will not
   * perform well and should not be used as part of the default code path in the
   * game loop.
   *
   ...
   * chance to render something else between lines (@see
   ...
   * {@link #renderedLine(int, int, int)}
   *
   * TODO: [Isometric map] Render stuff between lines, concept of line differs
   * from ortho maps
   */
   ...
   * @param tileSetsLocation The location from which we can retrieve tileset
   * images
   * @throws SlickException Indicates a failure to parse the map or find a
   * tileset
   */
